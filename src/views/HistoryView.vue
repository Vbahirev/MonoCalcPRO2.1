                    ...
                    <Transition mode="out-in" name="grid-fade">
                        <div :key="listKey" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-auto relative">
                            <div v-for="p in displayList" :key="p.id" 
                                 @click="askToLoad(p)"
                                 class="group bg-white dark:bg-[#1C1C1E] rounded-3xl p-5 shadow-[0_10px_30px_-10px_rgba(0,0,0,0.08)] dark:shadow-black/40 relative overflow-hidden flex flex-col h-40 md:h-48 transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 cursor-pointer w-full"
                            ...
                            >
                                <template v-if="!p.isPlaceholder">
                                    ...
                                    <div class="relative z-10 flex-1 flex items-start pr-6"><h3 class="text-sm md:text-base font-black text-[#1d1d1f] dark:text-white leading-snug line-clamp-2 break-words">{{ p.name }}</h3></div>
